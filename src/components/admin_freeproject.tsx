...
export default function AdminFreeProject() {
  ...
  useEffect(() => {
    async function fetchProjects() {
      try {
        const res = await fetch('/api/getfreeproject');
  ...
  const onSubmit: SubmitHandler<FormData> = async (data) => {
    ...
    try {
      const response = await fetch("/api/uploadfreeproject", {
        method: "POST",
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        setMessage(`Площадь загружена успешно с ID: ${result.itemId}`);
        alert(`Площадь загружена успешно с ID: ${result.itemId}`);
        reset();
        setBlogs([...blogs, {
          id: result.itemId,
  ...
  async function deleteProject(id: string) {
    try {
      const response = await fetch('/api/deletfreeproject', {
        method: 'DELETE',
      ...
      if (response.ok) {
        ...
        console.error('Ошибка при удалении площади:', await response.text());
      }
    } catch (error) {
      console.error('Ошибка при удалении площади:', error);
    }
  }

  const ProjectComponent = () => (
    <section className="relative flex bg-white h-max z-1 w-full justify-center py-32 bg-white">
      <div className="max-w-screen-lg py-8 bg-white">
            ...
            >
              ...
              <div className="p-6">
                <h3 className="text-xl font-bold mb-2">
                  <a
                    href={blog.link}
                    className="text-black-600 duration-300 text-black"
                  >
                    {blog.title}
                  </a>
                </h3>
  ...
  return (
    <div>
      <form className="form bg-white" onSubmit={handleSubmit(onSubmit)}>
        <div className="container mx-auto mt-28 py-36 bg-white">
